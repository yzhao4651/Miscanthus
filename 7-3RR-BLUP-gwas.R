...
myGD <- read.csv("data/myGD.136.3387.csv",row.names=1)
myGM <- read.csv("data/myGM.136.3387.csv")
myQ<- read.csv("data/myQ.136.3387.csv",row.names=1)
str(myQ)
...
gwasResults.136 <- GWAS(myY, genorrblup, fixed=NULL, K=kmatrix, n.PC=3,
                     min.MAF=0.05, n.core=1, P3D=TRUE, plot=FALSE)
write.csv(gwasResults.136, file = "rrBLUP/rrBLUP_GWAS_results.136.csv")

###this one for qq plots
out_start=4
out_end=41
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP136",
                      paste("136 QQ plot of VAR_", names(gwasResults.136[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  par(mar = c(5,4,1,1))
  qq(10 ^ -gwasResults.136[[i]])
  dev.off()
}
...
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP136",
                      paste("136 Manhattan plots of _", names(gwasResults.136[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  manhattan(data.frame(SNP = gwasResults.136$Name,
                       CHR = gwasResults.136$Chromosome,
                       BP = gwasResults.136$Position,
                       P = 10 ^ -gwasResults.136[[i]]), genomewideline = FALSE, suggestiveline = FALSE)
  dev.off()
}
