...
###Culm127
myY <- read.csv("data/myYC.127.2216.csv")
str(myY)
myGD <- read.csv("data/myGDC.127.2216.csv", row.names=1)
myGM <- read.csv("data/myGMC.127.2216.csv")
myQ<- read.csv("data/myQC.127.2216.csv", row.names=1)
str(myQ)
###tansform the genotype data set into rrblup data set
myGDt <- t(myGD)-1
#myGDtdataframe <- data.frame(myGDt)
...
kmatrix <- A.mat(myGDk,min.MAF=0.05, max.missing=NULL,impute.method="mean",tol=0.02,
                 n.core=1,shrink=FALSE,return.imputed=FALSE)
####do GWAS analysis again with Kmatrix
gwasResults.C127 <- GWAS(myY, genorrblup, fixed=NULL, K=kmatrix, n.PC=3,
                         min.MAF=0.05, n.core=1, P3D=TRUE, plot=FALSE)
write.csv(gwasResults.C127, file = "rrBLUPC127/rrBLUP_GWAS_results.C127.csv")
str(gwasResults.C127)

###this one for qq plots
out_start=4
out_end=17
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUPC127",
                      paste("C127 QQ plot of VAR_", names(gwasResults.C127[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  par(mar = c(5,4,1,1))
  qq(10 ^ -gwasResults.C127[[i]])
  dev.off()
}
###this one for Manhattan plots
out_start=4
...
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUPC127",
                      paste("C127 Manhattan plots of _", names(gwasResults.C127[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  manhattan(data.frame(SNP = gwasResults.C127$Name,
                       CHR = gwasResults.C127$Chromosome,
                       BP = gwasResults.C127$Position,
                       P = 10 ^ -gwasResults.C127[[i]]), genomewideline = FALSE, suggestiveline = FALSE)
  dev.off()
}
