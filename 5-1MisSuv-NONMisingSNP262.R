...
SNPnmrow <- SNP[-which(rowMeans(is.na(SNP)) > 0.262),]
...
myGD.116.4685 <- Select.MAF(SNPnmcol)
...
identical(as.character(myGM$Name), colnames(myGD.116.4685))
source("Function/getmyGM.R")
source("Function/getmyGD.R")
myGD.116.4685 <- getmyGD(myGM,myGD.116.4685)
str(myGD.116.4685)
myGM.116.4685 <- getmyGM(myGM,myGD.116.4685)
myGM.116.4685$Name
str(myGM.116.4685)
...
##############step5 myY: Phenotype data for GAPIT and FarmCPU 
myY.116.4685 <- allblup[match(myGD.116.4685$Taxa,allblup$Taxa, nomatch=0),]
myY.116.4685 <- droplevels(myY.116.4685)
######### order the Taxa
...
myY.116.4685$Taxa %in% myQ$Taxa
###elect myQ dataset using the phenotype data
myQ.116.4685 <- myQ[match(myY.116.4685$Taxa, myQ$Taxa, nomatch=0),]
##check the format
