...
###step 3 load GM data sets to select matched Genotype from step 2
load("~/Documents/161025forGAPIT.RData")
### change "." to"-" becasue they have the same name, but the string inside is different. 
...
##depending on the missing value and then separate the data to four 
str(myYmrMlMM)
myYmrMlMMSuv <- myYmrMlMM[1:2]
str(myYmrMlMMSuv)
names(myYmrMlMMSuv)
write.csv(myYmrMlMMSuv, file = "mrMLMM2/myYmrMlMMSuv2.csv", row.names = FALSE, na = "NA")
...
##change names 
myYmrMlMMflo <- colRename(myYmrMlMMflo)
names(myYmrMlMMflo)
write.csv(myYmrMlMMflo, file = "mrMLMM2/myYmrMlMMflo4.csv", row.names = FALSE, na = "NA")
###culm
###culm
...
##change names 
myYmrMlMMculm <- colRename(myYmrMlMMculm)
###check the column name if changed 
names(myYmrMlMMculm)
write.csv(myYmrMlMMculm, file = "mrMLMM2/myYmrMlMMculm4.csv", row.names = FALSE, na = "NA")
