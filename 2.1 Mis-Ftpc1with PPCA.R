...
fprind <- cbind(ranefvaluef[1:3], fprin_comp$x[,1])
names(fprind)
...
dev.off()
###get pc1 for each individual
fprinW <- cbind(ranefvaluef[1:3], fprin_comp$x[,1])
names(fprinW)
...
dev.off()
###get pc1 for each individual
fprinGW <- cbind(ranefvaluef[1:3], fprin_comp$x[,1])
names(fprinGW)
...
dev.off()
###get pc1 for each individual
fprinM <- cbind(ranefvaluef[1:3], fprin_comp$x[,1])
names(fprinM)
