...
myGD <- read.csv("data/myGD.129.4033.csv",row.names=1)
myGM <- read.csv("data/myGM.129.4033.csv")
myQ<- read.csv("data/myQ.129.4033.csv",row.names=1)
str(myQ)
...
gwasResults.129 <- GWAS(myY, genorrblup, fixed=NULL, K=kmatrix, n.PC=3,
                     min.MAF=0.05, n.core=1, P3D=TRUE, plot=FALSE)
write.csv(gwasResults.129, file = "rrBLUP/rrBLUP_GWAS_results.129.csv")

###this one for qq plots
out_start=4
out_end=41
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP129",
                      paste("129 QQ plot of VAR_", names(gwasResults.129[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  par(mar = c(5,4,1,1))
  qq(10 ^ -gwasResults.129[[i]])
  dev.off()
}
...
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP129",
                      paste("129 Manhattan plots of _", names(gwasResults.129[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  manhattan(data.frame(SNP = gwasResults.129$Name,
                       CHR = gwasResults.129$Chromosome,
                       BP = gwasResults.129$Position,
                       P = 10 ^ -gwasResults.129[[i]]), genomewideline = FALSE, suggestiveline = FALSE)
  dev.off()
}
