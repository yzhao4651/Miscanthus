...
### Remove the row with 35% missing value (more than 50% NA)
SNPnmrow <- SNP[-which(rowMeans(is.na(SNP)) > 0.35),]
...
myGD.146.2728 <- Select.MAF(SNPnmcol)
...
identical(as.character(myGM$Name), colnames(myGD.146.2728)[-1])
source("Function/getmyGM.R")
source("Function/getmyGD.R")
myGD.146.2728 <- getmyGD(myGM,myGD.146.2728)
myGM.146.2728 <- getmyGM(myGM,myGD.146.2728)
###check if they are shared the same name in the same order
identical(as.character(myGM.146.2728$Name), colnames(myGD.146.2728)[-1])
write.csv(myGM.146.2728, file = "data/myGM.146.2728.csv",row.names = FALSE)
write.csv(myGD.146.2728, file = "data/myGD.146.2728.csv",row.names = FALSE)


##############step5 myY: Phenotype data for GAPIT and FarmCPU 
##############step5 myY: Phenotype data for GAPIT and FarmCPU 
myY.146.2728 <- allblup[match(myGD.146.2728$Taxa,allblup$Taxa, nomatch=0),]
myY.146.2728 <- droplevels(myY.146.2728)
######### order the Taxa
#myYorderg <- na.omit(myYorderg[order(myYorderg$Taxa),])
str(myY.146.2728)
#### write out the phenotype with correct TAXA
write.csv(myY.146.2728, file = "data/myY.146.2728.csv", row.names = FALSE, na = "")
...
myY.146.2728$Taxa %in% myQ$Taxa
###elect myQ dataset using the phenotype data
myQ.146.2728 <- myQ[match(myY.146.2728$Taxa, myQ$Taxa, nomatch=0),]
##check the format
#str(myQ.clum.107.3707)
####write out the dataset
write.csv(myQ.146.2728, file = "data/myQ.146.2728.csv",row.names = FALSE)
