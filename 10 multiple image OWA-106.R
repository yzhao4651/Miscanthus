...
GLM <- read.csv("Result GAPIT1/106OWAGLMMLMMLMMFarmCPUCV/GAPIT.GLM.OWA.GWAS.Results.csv")
GLM <- GLM[1:4]
colnames(GLM)[colnames(GLM)=="P.value"] <- "GLM.OWA"
MLM <- read.csv("Result GAPIT1/106OWAGLMMLMMLMMFarmCPUCV/GAPIT.MLM.OWA.GWAS.Results.csv")
MLM <- MLM[,c(1,4)]
colnames(MLM)[colnames(MLM)=="P.value"] <- "MLM.OWA"
CMLM <- read.csv("Result GAPIT1/106OWAECMMLCV/GAPIT.CMLM.OWA.GWAS.Results.csv")
CMLM <- CMLM[,c(1,4)]
colnames(CMLM)[colnames(CMLM)=="P.value"] <- "CMLM.OWA"
MLMSUPER <- read.csv("Result GAPIT1/106OWAMLMSUPPER/GAPIT.SUPER.OWA.GWAS.Results.csv")
MLMSUPER<- MLMSUPER[,c(1,4)]
colnames(MLMSUPER)[colnames(MLMSUPER)=="P.value"] <- "SUPER.OWA"
gwasResultsqq <- read.csv("rrBLUPS106/rrBLUP_GWAS_results.O106.csv",row.names = 1)
...
setwd("/Users/yonglizhao/Documents/R-corde for miscanthus project/Miscanthus")
setwd("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/")
FarmCPU <- read.csv("Result GAPIT1/106OWAGLMMLMMLMMFarmCPU/GAPIT.FarmCPU.OWA.GWAS.Results.csv")
FarmCPU <- FarmCPU[1:4]
colnames(FarmCPU)[colnames(FarmCPU)=="P.value"] <- "FarmCPU.OWA"
MLMM <- read.csv("Result GAPIT1/106OWAGLMMLMMLMMFarmCPU/GAPIT.MLMM.OWA.GWAS.Results.csv")
...
setwd("/Users/yonglizhao/Documents/R-corde for miscanthus project/Miscanthus")
setwd("C:/Users/Admin/Desktop/Miscanthus/Miscanthus")
GLM <- read.csv("Result GAPIT1/126OWAGLMMLMMLMMFarmCPUCV/GAPIT.GLM.OWA.GWAS.Results.csv")
GLM <- GLM[1:4]
colnames(GLM)[colnames(GLM)=="P.value"] <- "GLM.OWA"
MLM <- read.csv("Result GAPIT1/126OWAGLMMLMMLMMFarmCPUCV/GAPIT.MLM.OWA.GWAS.Results.csv")
MLM <- MLM[,c(1,4)]
colnames(MLM)[colnames(MLM)=="P.value"] <- "MLM.OWA"
CMLM <- read.csv("Result GAPIT1/126OWAECMMLCV/GAPIT.CMLM.OWA.GWAS.Results.csv")
CMLM <- CMLM[,c(1,4)]
colnames(CMLM)[colnames(CMLM)=="P.value"] <- "CMLM.OWA"
MLMSUPER <- read.csv("Result GAPIT1/126OWAMLMSUPPER/GAPIT.SUPER.OWA.GWAS.Results.csv")
MLMSUPER<- MLMSUPER[,c(1,4)]
colnames(MLMSUPER)[colnames(MLMSUPER)=="P.value"] <- "SUPER.OWA"
gwasResultsqq <- read.csv("rrBLUPS126/rrBLUP_GWAS_results.O126.csv",row.names = 1)
gwasResultsqq <- gwasResultsqq[, c(1,5)]
colnames(gwasResultsqq)[colnames(gwasResultsqq)=="Name"] <- "SNP"
names(gwasResultsqq)
###merge all of them 
...
str(p.rrBLUP)
total <- do.call("rbind", list(p.GLM, p.CMLM, p.SUPER, p.rrBLUP))
total$SNP.N <- paste("126")
total <- total[,c(7,6,5,1:4)]
str(total)
###write the result
if (nrow(total)>0) {
  write.csv(total, file="Owaresults126/OWA_1.csv",row.names = F)
}
###plot QQ plot inone image
setwd("Allimage126OWA")
source("https://raw.githubusercontent.com/YinLiLin/R-CMplot/master/R/CMplot.r")
CMplot(plot,plot.type="q",col=c("blue", "orange", "cyan","magenta","green3"),threshold=1.13e-5,
       signal.pch=19,cex=0.5,signal.cex=1.0, signal.col="blue",conf.int.col="grey",box=FALSE,multracks=
         TRUE,file="jpg",memo="",dpi=300,
...
setwd("/Users/yonglizhao/Documents/R-corde for miscanthus project/Miscanthus")
setwd("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/")
FarmCPU <- read.csv("Result GAPIT1/126OWAGLMMLMMLMMFarmCPU/GAPIT.FarmCPU.OWA.GWAS.Results.csv")
FarmCPU <- FarmCPU[1:4]
colnames(FarmCPU)[colnames(FarmCPU)=="P.value"] <- "FarmCPU.OWA"
MLMM <- read.csv("Result GAPIT1/126OWAGLMMLMMLMMFarmCPU/GAPIT.MLMM.OWA.GWAS.Results.csv")
#MLMM <- read.csv("Result GAPIT1/MLMM/GAPIT.MLMM.OWA.GWAS.Results.csv")
MLMM <- MLMM[,c(1,4)]
colnames(MLMM)[colnames(MLMM)=="P.value"] <- "MLMM.OWA"
intermediate <- read.csv("mrMLMM2/OWA126/1_intermediate result.csv")
intermediate$X.log10.P. <- 10^-(intermediate$X.log10.P.)
...
###merge all of them 
plot <-  plyr::join_all(list(FarmCPU,MLMM,mrMLM,FASTmrMLM), type = "left", by="SNP")
plot[is.na(plot)] <- 1
...
MLMM$Trait.name <- paste("OWA")
MLMM <- MLMM[,c(6,5,1:4)]
mrmlm.final <- read.csv("mrMLMM2/OWA126/1_Final result.csv")
colnames(mrmlm.final)[colnames(mrmlm.final) == "RS."] <- "SNP"
colnames(mrmlm.final)[colnames(mrmlm.final) == "Marker.Position..bp."] <- "Position"
levels(mrmlm.final$Trait.name)[levels(mrmlm.final$Trait.name) == "Trait1"] <- "OWA"
mrmlm.final$P.value <- 10^-(mrmlm.final$X.log10.P.)
...
total <- unique(total[,c(7,1:6)])
###write the result
write.csv(total, file="Owaresults126/OWA_2.csv",row.names = F)
setwd("Allimage126OWA")
#library("CMplot")
source("https://raw.githubusercontent.com/YinLiLin/R-CMplot/master/R/CMplot.r")
CMplot(plot,plot.type="q",col=c("blue", "orange", "cyan","magenta","green3"),threshold=0.05/nrow(plot),
...
setwd("/Users/yonglizhao/Documents/R-corde for miscanthus project/Miscanthus")
setwd("C:/Users/Admin/Desktop/Miscanthus/Miscanthus")
GLM <- read.csv("Result GAPIT1/135OWAGLMMLMMLMMFarmCPUCV/GAPIT.GLM.OWA.GWAS.Results.csv")
GLM <- GLM[1:4]
colnames(GLM)[colnames(GLM)=="P.value"] <- "GLM.OWA"
MLM <- read.csv("Result GAPIT1/135OWAGLMMLMMLMMFarmCPUCV/GAPIT.MLM.OWA.GWAS.Results.csv")
MLM <- MLM[,c(1,4)]
colnames(MLM)[colnames(MLM)=="P.value"] <- "MLM.OWA"
CMLM <- read.csv("Result GAPIT1/135OWAECMMLCV/GAPIT.CMLM.OWA.GWAS.Results.csv")
CMLM <- CMLM[,c(1,4)]
colnames(CMLM)[colnames(CMLM)=="P.value"] <- "CMLM.OWA"
MLMSUPER <- read.csv("Result GAPIT1/135OWAMLMSUPPER/GAPIT.SUPER.OWA.GWAS.Results.csv")
MLMSUPER<- MLMSUPER[,c(1,4)]
colnames(MLMSUPER)[colnames(MLMSUPER)=="P.value"] <- "SUPER.OWA"
gwasResultsqq <- read.csv("rrBLUPS135/rrBLUP_GWAS_results.O135.csv",row.names = 1)
gwasResultsqq <- gwasResultsqq[, c(1,5)]
colnames(gwasResultsqq)[colnames(gwasResultsqq)=="Name"] <- "SNP"
names(gwasResultsqq)
###merge all of them 
...
str(p.rrBLUP)
total <- do.call("rbind", list(p.GLM, p.CMLM, p.SUPER, p.rrBLUP))
total$SNP.N <- paste("135")
total <- total[,c(7,6,5,1:4)]
str(total)
###write the result
if (nrow(total)>0) {
  write.csv(total, file="Owaresults135/OWA_1.csv",row.names = F)
}
###plot QQ plot inone image
setwd("Allimage135OWA")
source("https://raw.githubusercontent.com/YinLiLin/R-CMplot/master/R/CMplot.r")
CMplot(plot,plot.type="q",col=c("blue", "orange", "cyan","magenta","green3"),threshold=1.13e-5,
       signal.pch=19,cex=0.5,signal.cex=1.0, signal.col="blue",conf.int.col="grey",box=FALSE,multracks=
         TRUE,file="jpg",memo="",dpi=300,
...
setwd("/Users/yonglizhao/Documents/R-corde for miscanthus project/Miscanthus")
setwd("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/")
FarmCPU <- read.csv("Result GAPIT1/135OWAGLMMLMMLMMFarmCPU/GAPIT.FarmCPU.OWA.GWAS.Results.csv")
FarmCPU <- FarmCPU[1:4]
colnames(FarmCPU)[colnames(FarmCPU)=="P.value"] <- "FarmCPU.OWA"
MLMM <- read.csv("Result GAPIT1/135OWAGLMMLMMLMMFarmCPU/GAPIT.MLMM.OWA.GWAS.Results.csv")
#MLMM <- read.csv("Result GAPIT1/MLMM/GAPIT.MLMM.OWA.GWAS.Results.csv")
MLMM <- MLMM[,c(1,4)]
colnames(MLMM)[colnames(MLMM)=="P.value"] <- "MLMM.OWA"
intermediate <- read.csv("mrMLMM2/OWA135/1_intermediate result.csv")
intermediate$X.log10.P. <- 10^-(intermediate$X.log10.P.)
...
MLMM$Trait.name <- paste("OWA")
MLMM <- MLMM[,c(6,5,1:4)]
mrmlm.final <- read.csv("mrMLMM2/OWA135/1_Final result.csv")
colnames(mrmlm.final)[colnames(mrmlm.final) == "RS."] <- "SNP"
colnames(mrmlm.final)[colnames(mrmlm.final) == "Marker.Position..bp."] <- "Position"
levels(mrmlm.final$Trait.name)[levels(mrmlm.final$Trait.name) == "Trait1"] <- "OWA"
mrmlm.final$P.value <- 10^-(mrmlm.final$X.log10.P.)
...
total <- unique(total[,c(7,1:6)])
###write the result
write.csv(total, file="Owaresults135/OWA_2.csv",row.names = F)
setwd("Allimage135OWA")
#library("CMplot")
source("https://raw.githubusercontent.com/YinLiLin/R-CMplot/master/R/CMplot.r")
CMplot(plot,plot.type="q",col=c("blue", "orange", "cyan","magenta","green3"),threshold=0.05/nrow(plot),
