...
datacomb3tran <- data.frame(t(datacomb3))
### change row name to the first column 
rn <- rownames(datacomb3tran)
rownames(datacomb3tran) <- NULL
datacomb3tran <- cbind(rn,datacomb3tran)
###check the data
str(datacomb3tran)
...
allsnp2[,2] %in% as.character(datacomb3tran[,1])
#str(allsnp)
####find the datacomb2 data match the allsnpsub, pay attention the datacomb2tran should put the front of the %in%.
datacomb3transub <- subset(datacomb3tran, (as.character(datacomb3tran[,1]) %in% allsnp2[,2]))
###select several column from all of the allsnps
allsnp2sub <- data.frame(allsnp2[,c(1:2,3:7)])
str(allsnp2sub)
#### merge allsnpsub and datacomb2transub
datacomb3tranmerge <- merge(allsnp2sub,datacomb3transub,by="rn")
##checking the data
str(datacomb3tranmerge)
### get the genotype with the genotype.for. code.1 for using later on 
datacomb3tranmerge3 <- datacomb3tranmerge[,c(1,5,9:582)]
###check the data again
str(datacomb3tranmerge3)
###  writting out the data set datacomb2tranmerge2
write.csv(datacomb3tranmerge3, file = "data/datacomb3tranmerge3.csv", row.names = FALSE, na = "NA")
...
str(myGM)
myGM$Name %in% datacomb3tranmerge3$rn
###get all the match with the datacomb2tranmerge
myGM2 <-myGM[match(datacomb3tranmerge3$rn,myGM$Name, nomatch=0),] 
...
datacomb3tranmerge4 <- merge(myGM2,datacomb3tranmerge3,by="rn")
snporder <- order(datacomb3tranmerge4$Chromosome, datacomb3tranmerge4$Position)
datacomb3tranmerge4 <- datacomb3tranmerge4[snporder,]
###check 
str(datacomb3tranmerge4)
###write out the data, so far, we get all of the SNPs with all of the individuals with Chromosome, position and allele
write.csv(datacomb3tranmerge4, file = "data/datacomb3tranmerge4.csv", row.names = FALSE, na = "NA")
