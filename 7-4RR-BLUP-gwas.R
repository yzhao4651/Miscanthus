...
myGD <- read.csv("data/myGD.146.2728.csv",row.names=1)
myGM <- read.csv("data/myGM.146.2728.csv")
myQ<- read.csv("data/myQ.146.2728.csv",row.names=1)
str(myQ)
...
gwasResults.146 <- GWAS(myY, genorrblup, fixed=NULL, K=kmatrix, n.PC=3,
                     min.MAF=0.05, n.core=1, P3D=TRUE, plot=FALSE)
write.csv(gwasResults.146, file = "rrBLUP/rrBLUP_GWAS_results.146.csv")

###this one for qq plots
out_start=4
out_end=41
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP146",
                      paste("146 QQ plot of VAR_", names(gwasResults.146[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  par(mar = c(5,4,1,1))
  qq(10 ^ -gwasResults.146[[i]])
  dev.off()
}
...
for (i in out_start:out_end){
  mypath <- file.path("C:/Users/Admin/Desktop/Miscanthus/Miscanthus/rrBLUP146",
                      paste("146 Manhattan plots of _", names(gwasResults.146[i]), ".jpeg", sep = ""))
  jpeg(file=mypath) 
  require(qqman)
  manhattan(data.frame(SNP = gwasResults.146$Name,
                       CHR = gwasResults.146$Chromosome,
                       BP = gwasResults.146$Position,
                       P = 10 ^ -gwasResults.146[[i]]), genomewideline = FALSE, suggestiveline = FALSE)
  dev.off()
}
